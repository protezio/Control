...
public class GPIO {
	...
	public PORTVALUE GetValue(int port) {
		...
		return diomerda.ports.get(port).value;
	}
...
//	JSONObject response;
	...
	public void callMacro(final String macro, final String arg) {
		final GPIO self = this;
	   final JSONParser parser = new JSONParser(conn.username,conn.password, conn.host);
	    
		Thread t = new Thread(new Runnable() 
		{
			@Override
			public void run() {
				Log.i("BAR","http://" + String.valueOf(conn.host) + ":" + String.valueOf(conn.port) + "/macros"+"/"+ macro + "/" + arg);
				Log.i("BAR","Call Macro");
				try {
					
					
					obj = parser.postJSONFromURL("http://"+ conn.host + ":" + conn.port + "/macros"+"/"+ macro + "/" + arg, conn.port);
					Log.i("BAR","response:" +  obj.toString());					
	...
	public void UpdateGPios() {
		...
		final JSONParser parser = new JSONParser(conn.username, conn.password,
				conn.host);
		...
		try {
				...
				if (obj != null) {
					if(obj.has("UART0")) {
						diomerda.UARTEnabled = (obj.getInt("UART0") == 0) ? false
							: true;
					} else {
						...
						if(JSONPort.getString("function").equalsIgnoreCase("alt0")||
							...
							diomerda.UARTEnabled = true;
						}
					
					}
					if(obj.has("SPI0")) {
						diomerda.SPIEnabled = (obj.getInt("SPI0") == 0) ? false
							: true;
					} else {
						JSONObject values = obj.getJSONObject("GPIO");
						JSONObject JSONPort = values.getJSONObject("18");
						if(JSONPort.getString("function").equalsIgnoreCase("alt4")) {
							diomerda.SPIEnabled = true;
						}
					}
					if(obj.has("I2C0")||obj.has("I2C1")) {
						diomerda.I2CEnabled = ((obj.getInt("I2C0") == 0) ? false
							: true)
			...
			/*	if(firstRun) {
				...
				for (int i = 0; i < GPIO.TOTAL_PORTS; i++) {
					...
					diomerda.ports.set(i, p);
				}
				
				diomerda.map =  map;
				Log.i("DATA","class GPIO_22: " +   diomerda.ports.get(22).value);
				Log.i("DATA","class GPIO_23: " +   diomerda.ports.get(23).value);
				Log.i("DATA","class GPIO_24: " +   diomerda.ports.get(24).value);
				Log.i("DATA","class GPIO_27: " +   diomerda.ports.get(27).value);
